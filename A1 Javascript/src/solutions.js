...
function createLinkTag(rel, href) {
  ...
  return `<link rel=${relQuoted} href=${hrefQuoted}>`;
}
...
function parseDateString(value) {
  ...
  let match;
  if ((match = value.match(regex1)) !== null) {
    const year = parseInt(match[1], 10);
    const month = parseInt(match[2], 10) - 1;
    const day = parseInt(match[3], 10);
    return new Date(year, month, day);
  } else if ((match = value.match(regex2)) !== null) {
...
function normalizeTime(value) {
  ...
  // Return the formatted string in 24-hour time format
...
function buildApiEndpoint(resourceType, resourceId, extraData) {
  // Input validation for resourceType and resourceId
  if (!resourceType || !resourceId) {
    throw new Error('Resource type and resource ID must be provided and non-empty.');
  }

  // Start building the API endpoint
  let endpoint = `/${resourceType}/${resourceId}`;

  // If extraData is provided, append it to the endpoint after encoding
  if (extraData !== undefined) {
    if (extraData === '') {
      throw new Error('Extra data, if provided, must be non-empty.');
    }
